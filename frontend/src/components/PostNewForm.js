...
export default function PostNewForm() {
  ...
  const handleFinish = async (fieldValues) => {
    ...
    try {
      ...
      console.log("succes reponse :", response);
      history.push("/");
    } catch (error) {
      if (error.response) {
        ...
        if (typeof fieldsErrorMessages === "string") {
          notification.open({
            message: "서버 오류",
            description: `에러) ${status} 응답을 받았습니다. 서버에러 확인해주세요.`,
  ...
  return (
    <Form {...layout} onFinish={handleFinish} autoComplete={"false"}>
      ...
      <Form.Item
        label="Location"
        name="location"
        rules={[{ required: true, message: "Location을 입력해 주세요." }]}
        hasFeedback
      ...
      <Form.Item
        label="Photo"
        name="photo"
        rules={[{ required: true, message: "사진을 입력해주새요." }]}
        hasFeedback
        {...fieldErrors.photo}
      >
        <Upload
